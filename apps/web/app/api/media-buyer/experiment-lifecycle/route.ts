...
import { createPausedCanary, EXPERIMENT_LIFECYCLE_VERSION, listExperiments, overrideExperiment, type OwnerOverrideAction } from "@/lib/media-buyer/experiment-lifecycle";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

export async function GET(request: NextRequest) {
  const experiments = await listExperiments(request.nextUrl.searchParams.get("campaignDraftId")?.trim() || undefined);
  return NextResponse.json({ ok: true, engine: EXPERIMENT_LIFECYCLE_VERSION, experiments, safety: { canaryDeliveryStatus: "PAUSED", activationExecuted: false, realSpendUsed: false } });
}
