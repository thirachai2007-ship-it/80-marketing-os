...
export async function getSpec55Evidence() {
  const [drafts, decisions, experimentLogs, performanceDecisions, learningDecisions] = await Promise.all([
  ...
  if (drafts.length === 0) gaps.push({ reason: "NO_CAMPAIGN_WITH_AUDIENCE_USAGE" });
  ...
  if (latestExperiments.size === 0) gaps.push({ reason: "NO_AB_OR_DYNAMIC_EXPERIMENT" });
  ...
  return { evidenceVersion: SPEC_55_EVIDENCE_VERSION, requirement: "AI continuously adapts Campaign, Ad Set, budget allocation, bid, schedule, placement, creative and experiments for each Audience to maximize Net Profit, Revenue and scale", status: pass ? "PASS_REAL" : "NOT_PROVEN", pass, productionData: { audienceEnabledCampaigns: drafts.length, compliantAudienceCampaigns: drafts.filter((draft) => !gaps.some((gap) => gap.campaignDraftId === draft.id)).length, experiments: latestExperiments.size, experimentStatuses: [...latestExperiments.values()].reduce<Record<string, number>>((acc, item) => ({ ...acc, [item.status]: (acc[item.status] ?? 0) + 1 }), {}), performanceDecisions, learningDecisions, campaigns: drafts.map((draft) => ({ id: draft.id, campaignName: draft.campaignName, status: draft.status, audienceUsages: draft.audienceUsages.length, audienceTypes: [...new Set(draft.audienceUsages.map((usage) => usage.audienceAsset.audienceType))], allocationPercent: draft.audienceUsages.reduce((sum, usage) => sum + (usage.allocationPercent ?? 0), 0), audienceBudgetSatang: draft.audienceUsages.reduce((sum, usage) => sum + (usage.budgetSatang ?? 0), 0), forecastDailyBudgetSatang: draft.forecastDailyBudgetSatang, ads: draft.ads.length, creativeRevisions: draft.ads.filter((ad) => Boolean(ad.creativeRevisionId)).length })) }, optimizationScope: ["CAMPAIGN", "AD_SET", "BUDGET_ALLOCATION", "BID_STRATEGY", "SCHEDULE", "PLACEMENT", "CREATIVE", "AB_TEST", "DYNAMIC_OPTIMIZATION"], objectivePriority: ["NET_PROFIT", "REVENUE", "SCALE_CAPABILITY"], gapCount: gaps.length, gaps, safety: { ownerApprovalRequiredForRealChanges: true, metaMutationExecuted: false, budgetChanged: false, realSpendUsed: false, campaignPublished: false } };
}
