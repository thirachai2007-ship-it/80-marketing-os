...
type CreativeItem = {
  ...
  assetName: string; sourceContentId: string | null; originalPreviewUrl: string | null; pageName: string; productCategory: string; fingerprint: string;
};

function planSummary(item: CreativeItem) {
  const hook = item.editPlan?.timeline?.find((entry) => entry.operation === "HOOK")?.overlayText;
  const cta = item.editPlan?.timeline?.find((entry) => entry.operation === "CTA")?.overlayText ?? item.callToAction;
  return { hook: hook || "ยังไม่ได้ระบุ Hook", cta: cta || "ยังไม่ได้ระบุ CTA" };
}
...
export default function CreativeApprovalPanel() {
  ...
  const sourceCount = new Set(items.map((item) => item.sourceContentId ?? item.assetName)).size;
  ...
  return <section className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
    <div className="mb-5"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-violet-600">Creative Owner Approval</p><h2 className="mt-2 text-2xl font-semibold text-slate-950">เลือกแบบตัดต่อที่ต้องการ</h2><p className="mt-2 text-sm text-slate-600">คลิปต้นฉบับ {sourceCount} คลิป · แบบตัดต่อที่เสนอ {items.length} แบบ — ดูความต่างของ Hook, สัดส่วน, ความยาว และ CTA ก่อนอนุมัติ การอนุมัติยังไม่เปิดโฆษณาและไม่ใช้เงินโฆษณา</p></div>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{items.map((item) => {
      const sourceKey = item.sourceContentId ?? item.assetName;
      const variantNumber = (variantNumbers.get(sourceKey) ?? 0) + 1;
      variantNumbers.set(sourceKey, variantNumber);
      const summary = planSummary(item);
      ...
      <div className="space-y-3 p-4"><div className="flex items-center justify-between gap-2"><span className="rounded-full bg-violet-600 px-3 py-1 text-xs font-bold text-white">แบบที่ {variantNumber}</span><span className="text-xs text-slate-500">{item.editPlan?.aspectRatio ?? item.aspectRatio ?? "—"} · {((item.editPlan?.durationMs ?? item.durationMs ?? 0) / 1000) || "—"} วินาที</span></div>
        <div><p className="font-medium text-slate-950">{item.productCategory}</p><p className="text-xs text-slate-500">ต้นฉบับจาก {item.pageName}</p></div>
        <div className="space-y-2 rounded-xl bg-slate-50 p-3 text-sm"><p><span className="font-semibold text-slate-900">Hook:</span> <span className="text-slate-600">{summary.hook}</span></p><p><span className="font-semibold text-slate-900">CTA:</span> <span className="text-slate-600">{summary.cta}</span></p><p><span className="font-semibold text-slate-900">Placement:</span> <span className="text-slate-600">{item.editPlan?.placement ?? "ยังไม่ระบุ"}</span></p></div>
